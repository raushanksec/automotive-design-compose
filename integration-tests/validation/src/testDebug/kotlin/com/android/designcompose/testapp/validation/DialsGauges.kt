...
class DialsGauges {
    ...
    val roborazziRule =
        RoborazziRule(
            options =
                RoborazziRule.Options(outputDirectoryPath = "$ROBO_CAPTURE_DIR/DialsGauges")
        )
    ...
    @Test
    fun progressBarWidth() {
        with(composeTestRule) {
            //onNodeWithText("Triggers").performClick()
            //onNodeWithText("idle").performTouchInput { down(Offset.Zero) }
            onNodeWithTag("ProgressIndicator25").performClick()
            onNodeWithTag("progressIndicatorTest").assertExists()
            onNodeWithTag("progressIndicatorTest").assertLeftPositionInRootIsEqualTo(10.dp)

            onNodeWithTag("ProgressBar25").performClick()
            onNodeWithTag("progressBarTest").assertExists()

            onNodeWithTag("progressBarTest").assertWidthIsAtLeast(8000.dp)
            captureImg("progress-bar")
        }
    }
}
